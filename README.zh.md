...
| 命令 | 用途 |
|------|------|
| `/pm:create` | 從藍圖或描述建立議題 |
| `/pm:list` | 列出未結議題（按狀態分組）|
| `/pm:next` | 建議下一個優先任務 |
| `/pm:start` | 開始處理議題（worktree + 標籤）|
| `/pm:sync` | 推送進度更新至議題 |
| `/pm:close` | 以摘要關閉議題 |
| `/pm:status` | 專案儀表板 |
...
1. **建立** -- 從藍圖或描述生成議題（`/pm:create`）
2. **啟動** -- 選取議題，建立 worktree 與分支（`/pm:start`）
3. **同步** -- 開發過程中推送進度更新（`/pm:sync`）
4. **關閉** -- 以摘要完成議題（`/pm:close`）

## 整合

| 技能 | 關係 |
|------|------|
| `blueprint` | `/pm:create <blueprint>` 自動建立關聯議題 |
